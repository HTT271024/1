#!/usr/bin/env python3
# HTTP/1.1 Only Test Script for S0 baseline
# Modified from http_comparison.py to only run HTTP/1.1
...
RESULT_DIR = "/home/ekko/ns-3-dev-new/scratch/Comparison/results/http1_only"  # Results directory
    ...
    for i, metric in enumerate(metrics_to_plot):
        ...
        for size in file_sizes:
            if size in all_results and metric in all_results[size]:
                ...
                y_errors.append(all_results[size][metric]["stdev"])
            else:
                y_values.append(0)
                y_errors.append(0)
        ...
        bars = plt.bar(x, y_values, yerr=y_errors, capsize=5)
        ...
        for j, bar in enumerate(bars):
            ...
            if metric == "Avg_PageLoadTime":
                ...
                label = f'{height:.2f}'
            plt.text(bar.get_x() + bar.get_width()/2., height + y_errors[j] + (max(y_values) * 0.01),
