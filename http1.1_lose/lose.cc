...
//Setup 方法用于设置端口、响应包大小、最大请求数
//StartApplication：启动时创建 TCP socket，绑定端口，监听连接，并设置连接回调。
//StopApplication：关闭 socket。
private:
...
private:
  ...
  virtual void StopApplication() override {
    ...
    // 保存时间线数据到CSV文件
    std::ofstream outfile("/home/ekko/ns-3-dev-new/scratch/http1.1_lose/hol_timeline.csv");
    outfile << "request_id,send_time,recv_time\n";
    for (uint32_t i = 0; i < m_reqSendTimes.size(); ++i) {
      outfile << i << "," << m_reqSendTimes[i] << "," << m_respRecvTimes[i] << "\n";
    }
    outfile.close();
  }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "当前丢包率 errorRate = " << errorRate << std::endl;
