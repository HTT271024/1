...
# Define parameters
LATENCIES=("1ms" "2ms" "5ms" "10ms" "20ms")
BANDWIDTH="10Mbps"
LOSS="0.01"
N_REQUESTS="200"
RESP_SIZE="102400"
...
CSV="$OUTDIR/latency_sweep_h2.csv"
echo "latency_ms,bandwidth,loss,avg_delay_s,avg_throughput_Mbps,onload_s,retx_count,retx_rate_per_s,jitter_s,hol_events,hol_time_s,conn_hol_stall_s,conn_hol_ratio_percent,hpack_saved_bytes,hpack_compression_percent" > "$CSV"

# Loop through each latency
for LAT in "${LATENCIES[@]}"; do
    echo "Testing latency: $LAT"
    
    # Convert latency to milliseconds for CSV
    lat_ms=$(echo "$LAT" | sed 's/ms//')
    
    # Run simulation
    output=$(./ns3 run "$APP_PATH --dataRate=$BANDWIDTH --delay=$LAT --errorRate=$LOSS --nRequests=$N_REQUESTS --respSize=$RESP_SIZE --reqSize=$REQ_SIZE --interval=$INTERVAL --nConnections=$N_CONNECTIONS --nStreams=$N_STREAMS --frameChunk=$FRAME_CHUNK --tickUs=$TICK_US --headerSize=$HEADER_SIZE --hpackRatio=$HPACK_RATIO --connWindowMB=$CONN_WINDOW_MB --streamWindowMB=$STREAM_WINDOW_MB" 2>/dev/null)
    ...
    # Write data to CSV
    echo "$lat_ms,$BANDWIDTH,$LOSS,$avg_delay,$avg_throughput,$onload,$retx_count,$retx_rate,$jitter,$hol_events,$hol_time,$conn_hol_stall,$conn_hol_ratio,$hpack_saved,$hpack_compression" >> "$CSV"
    
    echo "Completed: $LAT"
done

echo "All latency experiments completed. Results saved to: $CSV" 