...
NS_LOG_COMPONENT_DEFINE("Http2LossTest");
...
public:
  void Setup(Address address, uint32_t payloadSize, uint32_t nStreams) {
    m_peer = address;
  ...
  void StartApplication() override {
    ...
    m_socket->SetSendCallback(MakeCallback(&SimpleHttp2Client::HandleSend, this));
    Simulator::Schedule(Seconds(1.0), &SimpleHttp2Client::Send, this);
  }
