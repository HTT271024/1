...
def run_test(args_tuple):
    ...
    print(f"===== [FINISHED] Test: {test_name}, Throughput: {throughput} Mbps, Duration: {duration:.2f}s =====")
    ...
    return {
        ...
        "duration": duration  # 添加duration到返回结果
    }
...
def main():
    ...
    parser.add_argument("--hol-only", action="store_true", help="Only run the HOL blocking test.")
    ...
    test_configs = [
        ...
        ("throughput_scaling_22KB", 22528, "100Mbps", "20ms", 0, 3, False, base_n_requests, args.verbose),  # 新增22KB测试
        ...
        # Test 2.2: HOL Blocking Elimination - 修改：丢包率设为0，增加测试时间
        ("hol_blocking", 51200, "100Mbps", "20ms", 0, 20, True, base_n_requests, args.verbose),
    ]
    
    # 如果指定了只运行HOL阻塞测试，则只保留该测试
    if args.hol_only:
        print("\n*** RUNNING ONLY HOL BLOCKING TEST ***\n")
        test_configs = [test for test in test_configs if test[0] == "hol_blocking"]
    ...
    # 新增: 创建并打印一个总结表
    if results:
        print("\n" + "="*25 + " Test Execution Summary " + "="*25)
        summary_df = pd.DataFrame(results)
        
        # 格式化列，让输出更好看
        summary_df['throughput'] = summary_df['throughput'].apply(
            lambda x: f"{x:.2f} Mbps" if x is not None else "N/A"
        )
        summary_df['duration'] = summary_df['duration'].apply(lambda x: f"{x:.2f}s")
        
        # 选择要显示的列，并按指定顺序排列
        display_columns = ['test_name', 'throughput', 'duration']
        # 使用 .to_string(index=False) 打印一个干净的表格，不带行号
